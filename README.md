...
- **[Hermetic](https://github.com/hermetic-sys/hermetic)** - Credential broker for AI agents. A local Rust daemon that holds API keys in an encrypted vault and makes authenticated HTTP requests on the agent's behalf — the agent never sees the credential.
